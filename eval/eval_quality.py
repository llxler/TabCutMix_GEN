...
# parser.add_argument('--path', type=str, default = "shoppers_sampled_-5_-20.csv", help='The file path of the synthetic data')


# args = parser.parse_args()

def eval_quality(real_data, syn_data, dataname, model):
...
if __name__ == '__main__':
    ...
    # syn_data = syn_data[:real_data.shape[0]]
    
    save_dir = f'eval/quality/{dataname}'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    Alpha_Precision_all, Beta_Recall_all = eval_quality(real_data, syn_data, dataname, model)

    with open(f'{save_dir}/tabddpm_-5_-20.txt', 'w') as f:
