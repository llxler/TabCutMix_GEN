...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def sample(self, num_samples):
        ...
        # 2. 路径和数据
        raw_config = src.load_config("/home/lxl/TabCutMix/baselines/tabddpm/configs/shoppers.toml")
        real_data_path = "data/shoppers"
        train_data = pd.read_csv("/home/lxl/TabCutMix/synthetic/shoppers/real.csv")
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            # if (i + 1) % 10 == 0 or i == 0:
                ...
                # mem = cal_memorization("shoppers", save_path, train_data)
                # mem_cat = cal_cat_mem("shoppers", save_path, train_data)
                # mem_num = cal_num_lxler("shoppers", save_path, train_data)
                
                # mem_list[i] = mem
                # mem_cat_list[i] = mem_cat
                # mem_num_list[i] = mem_num
                
                # print(f"在step: {i}的时候，memorization的值为: {mem}, cat_mem: {mem_cat}, num_mem: {mem_num}")
